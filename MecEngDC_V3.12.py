...
while True:
        ...
        #Linear motion dynamics calculator
                ...
                #Torsion bar springs calculator
        elif mode == 140:
                modeselected = ('Torsion bar springs')
                print(' ')
                print(modeselected)
                print(':::DATA INPUT:::')
                
                Lspring  = float(input("Available length (m)?: "))
                Stroke  = float(input("Required stroke (mm)?: "))
                Llever = float(input("Lever arm length (mm)?: "))
                G = float(input("Shear modulus (GPa)?: (80GPa typical steel)"))
                F = float(input("Load (N)?: "))
                S = float (input("Fatigue safety factor?: "))
                
               
                
                Mt = F * (Llever/1000)                                  #Torque
                alpha = (math.atan(Stroke/Llever)) * (180/ math.pi)     #Torsion angle in degrees
                Ipmin = (180*Mt*Lspring)/((math.pi)*alpha*(G*(10**9)))  #Required polar moment of inertia
                dmin = (((Ipmin)*32)/(math.pi))**(1/4)         #Required diameter
                tau = Mt / (((math.pi) * (dmin**3))/16)                 #Shear stress
                sigma = S * tau * math.sqrt (3)                         #Required yield strength of material
                ...
                print("         ",modeselected)
                print(' ')
                print("Spring length: ", Lspring, "m")
                print("Stroke: ", Stroke, "mm")
                print("Lever length: ", Llever, "mm")
                print("Material shear modulus: ", G, "GPa")
                print("Load: ", F, "N")
                print("Fatigue safety factor: ", S)
                print("Resulting torque: ", Mt, "Nm")
                print("Resulting torsion angle: ", alpha, "º")
                print("Required polar moment of inertia: ", Ipmin*10**8, "cm^4")
                print("Required diameter ", dmin*10**3, "mm")
                print("Resulting shear stress: ", tau*10**-6, "MPa")
                print("Resulting equivalent yield stress: ", sigma*10**-6, "MPa")
        ...
        if save=='y':
                if mode == 10:
                        ...
                        f = open(filename + ".txt",'w')
                        f.write(TitleName + '\n' + Version + '\n' + Author + '\n' + AuthorContact + '\n' + '\n' + 'Filename: ' + filename + '\n' + '\n' + SaveFileTitle + '\n' + '\n' + modeselected  + '\n' + '\n')
                        f.write('Spring length-------------------------------: ' + (str(Lspring)) + ' m' + '\n')
                        f.write('Stroke--------------------------------------: ' + (str(Stroke)) + ' mm' + '\n')
                        f.write('Lever length--------------------------------: ' + (str(Llever)) + ' mm' + '\n')
                        f.write('Material shear modulus----------------------: ' + (str(G)) + ' GPa' + '\n')
                        f.write('Load----------------------------------------: ' + (str(F)) + ' N' + '\n')
                        f.write('Fatigue safety factor-----------------------: ' + (str(S)) + '\n')
                        f.write('Resulting torque----------------------------: ' + (str(Mt)) + ' Nm' + '\n')
                        f.write('Resulting torsion angle---------------------: ' + (str(alpha)) + ' º' + '\n')
                        f.write('Required polar moment of inertia------------: ' + (str(Ipmin*10**8)) + ' cm^4' + '\n')
                        f.write('Required diameter---------------------------: ' + (str(dmin*10**3)) + ' mm' + '\n')
                        f.write('Resulting shear stress----------------------: ' + (str(tau*10**-6)) + ' MPa' + '\n')
                        f.write('Resulting equivalent yield stress-----------: ' + (str(sigma*10**-6)) + ' MPa ' + '\n'+ '\n'+ '\n')
                        f.write(CautionSentence)
                        f.close()
